...
def test_sample_source_rejects_invalid_target(
    tmp_path, window_title: str, backend: str
) -> None:
    source = FakeSource([])
...
def test_run_windows_sampling_rejects_invalid_start_delay(
    tmp_path, start_delay: object
) -> None:
...
def test_sample_frames_main_reports_runtime_error(monkeypatch, capsys, tmp_path) -> None:
    ...
    exit_code = sample_frames.main(
        ["--window-title", "三角洲行动", "--output", str(tmp_path)]
    )
