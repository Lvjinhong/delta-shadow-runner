...
最终程序分成两层，外层循环负责把每局可靠地串起来，内层只负责局内路线：
...
当前开发顺序也是先完成外层循环，再标定局内路线。任何未识别页面、教学页、弹窗或分辨率变化都会停止，不会沿用固定坐标继续点击。
...
Preflight 的 `SendInput` 只发给标题和 HWND 都经过校验的独立测试窗口；游戏阶段只截图。F12 仍是急停键。SessionSample、DryRun、Armed、SessionArmed、ControlledE2E 和 Preflight 不能并行启动，重复运行会被 Windows mutex 拒绝。
...
菜单 Profile 必须以实际客户端页面为准，不能沿用旧版本页面名称。2026-07-19 起，本机固定基线改为 1920×1080 窗口模式；起点是 3D 主页，顶部“开始游戏”，右下显示“零号大坝-常规”和“配装/出发”。旧的 2560×1440 模板、ROI 和坐标不得缩放复用。当前最短进图链应建模为：

```text
BASE → LOBBY/地图确认 → IN_MATCH
```

如果账号仍出现战略板、配装确认、零装备警告或教学页，必须为这些页面分别采集模板并显式加入状态链；不能把未知中间页当成已成功跳过。
